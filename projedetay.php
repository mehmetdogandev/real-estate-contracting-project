...
try {
	...
	$sorgu = "SELECT projeler.id, projeler.urunadi, projeler.aciklama, projeler.fiyat, projeler.giris_tarihi, projeler.dzltm_tarihi,
	...
	<div class="row">
		<div class="col-md-6">
			...
			try {
				...
				$sorgu = "SELECT resim_iki FROM projeler WHERE resim='{$resim}' LIMIT 0,1";
				$stmt = $con->prepare($sorgu);
				// slider-1 için sorguyu çalıştır
				$stmt->execute();
				//slider-2 için seçme sorgusunu hazırla
				$sorgu2 = "SELECT resim_uc FROM projeler WHERE resim='{$resim}' LIMIT 0,1";
				$stmt2 = $con->prepare($sorgu2);
				//slider-2 için sorguyu çalıştır
				$stmt2->execute();
				//slider-3 için seçme sorgusunu hazırla
				$sorgu3 = "SELECT resim_dort FROM projeler WHERE resim='{$resim}' LIMIT 0,1";
					...
					<div class="carousel-inner">
						...
						if ($sayi2 > 0) {
							...
							while ($kayit = $stmt2->fetch(PDO::FETCH_ASSOC)) {
								...
								extract($kayit);
							?>
								<div class="carousel-item <?php if ($sayi1 == 0) {
																echo "active";
															} else {
																echo "";
															} ?>">
						...
						if ($sayi3 > 0) {
							...
							while ($kayit = $stmt3->fetch(PDO::FETCH_ASSOC)) {
								...
								extract($kayit);
							?>
								<div class="carousel-item <?php if ($sayi1 == 0 && $sayi2 == 0) {
																echo "active";
															} else {
																echo "";
															} ?>">
			...
			<table class="table table-hover table-responsive" style="font-size:14px;">
				...
				<?php if ($evarsa_id == "3") { ?>
					...
					<tr>
						...
						<td <?php if ($ev_tipi == "Apartman Dairesi") {
								echo "colspan='2'";
							} ?>>
							<?php
							echo $ev_tipi;
...
<!--   Bu yapılan çalışmanın  ev ıd çekme noktası 

	<a href="msj_gonder.php?konu=<?php echo $favori_icin_id; ?>&kime=<?php echo $kayit2['id']; ?>">
					<div class="btn btn-danger pull-right" style="
  position: absolute;
  border-radius:10px;
  top: 20%;
  left: 65%;">
 ...
 -->
	...
	<h4 class="baslik mt-5"><b><?php echo $kategoriadi . " " . $evOarsa; ?></b> kategorisindeki diğer ilanlar</h2>
		<div class="row">
			...
			$sorgu = 'SELECT projeler.id, projeler.urunadi, projeler.aciklama, projeler.fiyat, projeler.resim, projeler_kategoriler.kategoriadi, il.sehir, ilce.ilce 
 FROM projeler 
 LEFT JOIN projeler_kategoriler ON projeler.kategori_id = projeler_kategoriler.id 
 LEFT JOIN il ON projeler.il_id=il.id
 LEFT JOIN ilce ON projeler.ilce_id=ilce.id
 WHERE projeler.onay="1" AND projeler_kategoriler.kategoriadi="' . $kategoriadi . '" AND projeler.evarsa_id="' . $evOarsa_id . '" AND projeler.id<>"' . $favori_icin_id . '" limit 0,4';
