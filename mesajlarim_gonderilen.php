...
$sorguKime = "SELECT id FROM kullanicilar WHERE eposta=:eposta LIMIT 1";
$stmtKime = $con->prepare($sorguKime);
$stmtKime->execute(['eposta' => $_SESSION["kullanici_loginkey"]]);
$kayitKime = $stmtKime->fetch(PDO::FETCH_ASSOC);

$sorguGelenMesaj = "SELECT k_msj_kime, MAX(k_msj_id) AS k_msj_id, k_msj_kimden, MAX(k_msj_konu) AS k_msj_konu
                    FROM kullanicilar_mesaj 
                    WHERE k_msj_kimden = :kimden 
                    GROUP BY k_msj_kime, k_msj_kimden 
                    ORDER BY k_msj_id DESC";
$stmtGelenMesaj = $con->prepare($sorguGelenMesaj);
$stmtGelenMesaj->execute(['kimden' => $kayitKime['id']]);
...
<div class="row mt-4">
 ...
 <div class='row justify-content-center'>
    ...
    ?>
        ...
        <td><?php
            $sorguMsjKimdenAdSoyad = "SELECT adsoyad FROM kullanicilar WHERE id=:kime LIMIT 1";
            $stmtMsjKimdenAdSoyad = $con->prepare($sorguMsjKimdenAdSoyad);
            $stmtMsjKimdenAdSoyad->execute(['kime' => $kayitGelenMesaj['k_msj_kime']]);
        ...
        $sorguMsjIcerik = "SELECT * FROM kullanicilar_mesaj 
                           WHERE k_msj_id < :msj_id 
                           AND k_msj_kimden = :kimden 
                           AND k_msj_kime = :kime 
                           AND k_msj_konu = :konu";
        $stmtMsjIcerik = $con->prepare($sorguMsjIcerik);
        $stmtMsjIcerik->execute([
            'msj_id' => $kayitGelenMesaj['k_msj_id'],
            'kimden' => $kayitGelenMesaj['k_msj_kime'],
            'kime' => $kayitGelenMesaj['k_msj_kimden'],
            'konu' => $kayitGelenMesaj['k_msj_konu']
        ]);
        ...
        if($sayMsjIcerik > 0){
            ...
            echo "<a href='msj_detay.php?id=".$kayitMsjIcerik['k_msj_id']."' class='btn btn-info m-r-1em'> <span class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
        }else{
            // kayıt detay sayfa bağlantısı
            echo "<a href='msj_detay_gonderilen.php?id=".$kayitGelenMesaj['k_msj_id']."' class='btn btn-info m-r-1em'> <span class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
        }
