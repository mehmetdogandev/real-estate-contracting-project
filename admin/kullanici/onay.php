...
<div class="container">
 ...
 echo "<div class='alert alert-success'>Kaydın üyeliği onaylandı.</div>";
 }
 else if($islem=='onaylanmadi'){
 echo "<div class='alert alert-danger'>Kaydın üyeliği onaylanmadı.</div>";
 }

 // sayfada görüntülenecek kayıtları seç
 $aranan = isset($_GET['aranan']) ? $_GET['aranan'] : "";
 $arama_sarti = isset($_GET['aranan']) ? "%".$_GET['aranan']."%" : "%";
 $sorgu = "SELECT * FROM kullanicilar WHERE onay='0' AND adsoyad LIKE :aranan ORDER BY id DESC LIMIT :ilk_kayit_no, :sayfa_kayit_sayisi";
 ...
 <input type="text" class="form-control" placeholder="Kişi ara..."
...
//tablo başlangıcı
 ...
 // kayıt silme butonu
 echo "<a href='#' onclick='silme_onay({$id});' class='btn btn-danger m-r-1em'> <span class='glyphicon glyphicon-remove-circle'></span> Sil</a>"; 
 // Kullanıcıyı Onaylama Butonu
 echo "<a href='kullanici_onay.php?id={$id}' class='btn btn-warning'> <span class='glyphicon glyphicon-ok'></span> Onayla</a>"; 
 echo "</td>";
 echo "</tr>";
 }
 ...
 $sorgu = "SELECT COUNT(*) as kayit_sayisi FROM kullanicilar WHERE adsoyad LIKE :aranan";
 $stmt = $con->prepare($sorgu);
 $stmt->bindParam(":aranan", $arama_sarti);
