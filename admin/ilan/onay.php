...
<div class="container">
 ...
 // eğer silme veya onay (sil.php veya yayin_onay.php) sayfasından yönlendirme yapıldıysa
 if($islem=='silindi'){
 ...
 else if($islem=='onaylandi'){
 echo "<div class='alert alert-success'>Ürün yayınlandı.</div>";
 }
 else if($islem=='onaylanmadi'){
 echo "<div class='alert alert-danger'>Ürün yayınlanmadı.</div>";
 }

// sayfada görüntülenecek kayıtları seç
 $aranan = isset($_GET['aranan']) ? $_GET['aranan'] : "";
 $arama_sarti = isset($_GET['aranan']) ? "%".$_GET['aranan']."%" : "%";
 $sorgu = "SELECT urunler.id, urunler.urunadi, urunler.resim, urunler.aciklama, urunler.fiyat, urunler.onay,
kategoriler.kategoriadi, il.sehir, ilce.ilce, evarsa.ilanTuru
FROM urunler 
...
WHERE urunler.onay='0' AND (urunler.id LIKE :aranan OR evarsa.ilanTuru LIKE :aranan) 
ORDER BY urunler.id DESC LIMIT :ilk_kayit_no, :sayfa_kayit_sayisi";
...
<span class='glyphicon glyphicon glyphicon-remove'></span> Seçilenleri Sil</a>";
 ...
 echo "<a href='ekle_secim.php' class='btn btn-primary m-b-1em col-xs col-md m-r-1em pull-left'> 
 <span class='glyphicon glyphicon glyphicon-plus'></span> Yeni İlan</a>";
...
method="get">
 ...
 <input type="text" class="form-control" placeholder="ID'ye göre ilan ara..."
...
//tablo başlangıcı
 ...
 //Açıklama satırını ve ürün adını belli bir uzunlukta tutmak ve html gibi kodlardan korumak için filtreler
 $aciklama = trim(strip_tags($aciklama));
 $urunadi = trim(strip_tags($urunadi));
 if(strlen($aciklama) > 30){
	$aciklama = substr($aciklama ,'0','30')."..."; 
 }
 if(strlen($urunadi) > 20){
	$urunadi = substr($urunadi ,'0','20')."..."; 
 }
 ...
 echo "<td>{$id}</td>";
 echo "<td>";if($resim){echo "<img src='../../content/images/".$resim."' alt='".$urunadi."' class='img-fluid img-thumbnail' width='80' />";}else{echo "<img src='../../content/images/gorsel-yok.jpg' class='img-fluid img-thumbnail' width='80' /></td>";}
 echo "<td>{$urunadi}</td>";
...
class='glyphicon glyphicon glyphicon-edit'></span> Düzelt</a>";
 // kayıt silme sayfa bağlantısı
echo "<a href='#' onclick='silme_onay({$id});' class='btn btn-danger m-r-1em'> <span
class='glyphicon glyphicon glyphicon-remove-circle'></span> Sil</a>";
// kayıt onaylama sayfa bağlantısı
echo "<a href='yayin_onay.php?id={$id}' class='btn btn-warning'> <span
class='glyphicon glyphicon glyphicon-ok'></span> Onayla</a>";
 echo "</td>";
 echo "</tr>";
 }
...
$sorgu = "SELECT COUNT(*) as kayit_sayisi FROM urunler WHERE onay='0' AND id LIKE :aranan";
$stmt = $con->prepare($sorgu);
$stmt->bindParam(":aranan", $arama_sarti);
