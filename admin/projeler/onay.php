...
<div class="container">
	...
	$sorgu = "SELECT projeler.id, projeler.urunadi, projeler.resim, projeler.aciklama, projeler.fiyat, projeler.onay,
kategoriler.kategoriadi, il.sehir, ilce.ilce, evarsa.ilanTuru
FROM projeler 
LEFT JOIN kategoriler ON projeler.kategori_id = kategoriler.id 
...
WHERE projeler.onay='0' AND (projeler.id LIKE :aranan OR evarsa.ilanTuru LIKE :aranan) 
...
class='glyphicon glyphicon glyphicon-ok'></span> Onayla</a>";
		...
		$sorgu = "SELECT COUNT(*) as kayit_sayisi FROM projeler WHERE onay='0' AND id LIKE :aranan";
		$stmt = $con->prepare($sorgu);
		$stmt->bindParam(":aranan", $arama_sarti);
