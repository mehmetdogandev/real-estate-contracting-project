...
$sorguKime = "SELECT id FROM kullanicilar WHERE eposta='" . $_SESSION["kullanici_loginkey"] . "' LIMIT 1";
$stmtKime = $con->prepare($sorguKime);
$stmtKime->execute();
$kayitKime = $stmtKime->fetch(PDO::FETCH_ASSOC);

$sorguGelenMesaj = "SELECT MAX(k_msj_id) as k_msj_id, k_msj_kimden, k_msj_konu FROM kullanicilar_mesaj WHERE k_msj_kime=? GROUP BY k_msj_kimden, k_msj_konu ORDER BY k_msj_id DESC";
$stmtGelenMesaj = $con->prepare($sorguGelenMesaj);
$stmtGelenMesaj->execute([$kayitKime['id']]);
...
<div class="container p-5">
	...
	<a href='mesajlarim_gonderilen.php' class='btn btn-info pull-left'>Gönderilen Mesajlara Bak</a>

	<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
		<div class="row">
			<div class="col-xs-6 col-md-8">
				<div class="input-group">
					<input type="text" class="form-control" placeholder="Mesaj ara...(İsim ya da mesaj aratabilirsiniz)" name="aranan" value="<?php //echo isset($_GET['aranan']) ? $_GET['aranan'] : ""; ?>" />
		...
		<div class="col-md-12">
			<div class='row justify-content-center'>
				<div class='col-auto'>
					<?php if ($sayGelenMesaj > 0) { ?>
						<table class="table table-hover table-responsive table-bordered favori-tablo">
							...
							foreach ($veriGelenMesaj as $kayitGelenMesaj) {
								$k_msj_kime = isset($kayitGelenMesaj['k_msj_kime']) ? $kayitGelenMesaj['k_msj_kime'] : "";
							?>
								<tr>
									<td><?php echo "<input type='checkbox' name='sil_id[]' value='" . $kayitGelenMesaj['k_msj_id'] . "'/>"; ?></td>
									<td><?php
										$sorguMsjKimdenAdSoyad = "SELECT adsoyad FROM kullanicilar WHERE id=? LIMIT 1";
										$stmtMsjKimdenAdSoyad = $con->prepare($sorguMsjKimdenAdSoyad);
										$stmtMsjKimdenAdSoyad->execute([$kayitGelenMesaj['k_msj_kimden']]);
									...
									<td>
										...
										$sorguMsjIcerik = "SELECT * FROM kullanicilar_mesaj WHERE k_msj_id < ? AND k_msj_kimden = ? AND k_msj_kime = ? AND k_msj_konu = ?";
										$stmtMsjIcerik = $con->prepare($sorguMsjIcerik);
										$stmtMsjIcerik->execute([$kayitGelenMesaj['k_msj_id'], $k_msj_kime, $kayitGelenMesaj['k_msj_kimden'], $kayitGelenMesaj['k_msj_konu']]);
										$sayMsjIcerik = $stmtMsjIcerik->rowCount();
										$kayitMsjIcerik = $stmtMsjIcerik->fetch(PDO::FETCH_ASSOC);
							
										if ($sayMsjIcerik > 0) {
											// kayıt detay sayfa bağlantısı
											echo "<a href='msj_detay_gonderilen.php?id=" . $kayitMsjIcerik['k_msj_id'] . "' class='btn btn-info m-r-1em'> <span class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
										} else {
											// kayıt detay sayfa bağlantısı
											echo "<a href='msj_detay.php?id=" . $kayitGelenMesaj['k_msj_id'] . "' class='btn btn-info m-r-1em'> <span class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
										}
							
										// kayıt sil
										echo "<a href='#' onclick='silme_onay(" . $kayitGelenMesaj['k_msj_id'] . ");' class='btn btn-danger'> <span class='glyphicon glyphicon glyphicon-remove-circle'></span> Mesaj Grubunu Sil</a>";
										?>
									</td>
								</tr>
					...
					<?php } else {
						echo "<div class='alert alert-success text-center'>Gelen kutunuz boş görünüyor...</div>";
					} ?>
				</div>
			</div>
		</div>
