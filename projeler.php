...
<div class="container mt-4">
	<div class="row">
		<div class="col-md-3">
			...
			<div><!--projeler_kategoriler-->
				<h4 class="baslik">proje kategorileri</h4>
				<ul class="list-group list-group-flush">
					...
					$sorgu = 'SELECT projeler_kategoriler.*, COUNT(projeler.id) AS adet FROM projeler_kategoriler LEFT JOIN projeler ON projeler_kategoriler.id=projeler.kategori_id WHERE projeler.onay="1" GROUP BY projeler_kategoriler.id ORDER BY projeler_kategoriler.kategoriadi';
					...
					foreach ($veri as $kayit) { ?>
						<a href="projeler.php?id=<?php echo $kayit["id"];
												echo $parametre ?>" class="list-group-item d-flex justify-content-between align-items-center list-group-item-action<?php echo $kategori == $kayit["id"] ? "active" : ""; ?>"> <?php echo $kayit["kategoriadi"]; ?>
							<span class="badge badge-success badge-pill"><?php echo $kayit["adet"] ?></span>
						</a>
					<?php $toplam += $kayit["adet"];
					} ?>
					<a href="projeler.php?id=<?php echo $parametre ?>" class="list-group-item d-flex justify-content-between align-items-center list-group-item-action<?php echo $kategori == "" ? "active" : ""; ?>">Hepsi
			...
			<div class="pt-4"><!--sıralama-->
				...
				<div class="list-group list-group-flush">
					...
					<a href="projeler.php?siralama=akilli<?php echo $parametre ?>" class="list-group-item list-group-item-action<?php echo $siralama == "akilli" ? "active" : "";
																																?>">Akıllı sıralama</a>
					<a href="projeler.php?siralama=yeni<?php echo $parametre ?>" class="list-group-item list-group-item-action<?php echo $siralama == "yeni" ? "active" : ""; ?>">Yeni
						ilanlar</a>
					<a href="projeler.php?siralama=artan<?php echo $parametre ?>" class="list-group-item list-group-item-action<?php echo $siralama == "artan" ? "active" : "";
																																?>">Artan fiyat</a>
					<a href="projeler.php?siralama=azalan<?php echo $parametre ?>" class="list-group-item list-group-item-action<?php echo $siralama == "azalan" ? "active" : "";
																																?>">Azalan fiyat</a>
				</div>
			</div><!--/sıralama-->
			...
			<div class="row">
				...
				switch ($siralama) {
					...
					case 'azalan':
						...
						break;
					default:
						$orderby = "dzltm_tarihi desc";
				}
				// select sorgusu için fiyat aralığı hazırlanıyor
				switch ($fiyat) {
					case '1':
						$where1 = "fiyat between 0 and 2500";
						break;
					case '2':
						$where1 = "fiyat between 2500 and 5000";
						break;
					case '3':
						$where1 = "fiyat between 5000 and 10000";
						break;
					case '4':
						$where1 = "fiyat between 10000 and 25000";
						break;
					case '5':
						$where1 = "fiyat between 25000 and 50000";
						break;
					case '6':
						$where1 = "fiyat between 50000 and 100000";
						break;
					case '7':
						$where1 = "fiyat between 100000 and 200000";
						break;
					case '8':
						$where1 = "fiyat between 200000 and 100000000";
						break;
				}
				// select sorgusu için evarsa_id aralığı hazırlanıyor
				switch ($evarsa_id) {
					case '1':
						$where_evarsa_id = "evarsa_id='3'";
						break;
					case '2':
						$where_evarsa_id = "evarsa_id='2'";
						break;
					case '3':
						$where_evarsa_id = "evarsa_id='3' or evarsa_id='2'";
						break;
				}
				...
				if ($aranan != "") $where3 = "urunadi like '%$aranan%'";
				else $where3 = "urunadi like '%'";
				// select sorgusu için şartlar birleştiriliyor
				if (isset($where1) && isset($where2) && isset($where_evarsa_id)) {
					$where = "(" . $where1 . ") and (" . $where2 . ") and (" . $where3 . ") and (" . $where_evarsa_id . ")";
				} elseif (isset($where2) && isset($where_evarsa_id)) {
					$where = "(" . $where_evarsa_id . ") and (" . $where2 . ") and (" . $where3 . ")";
				} elseif (isset($where1) && isset($where_evarsa_id)) {
					$where = "(" . $where1 . ") and (" . $where3 . ") and (" . $where_evarsa_id . ")";
				} elseif (isset($where1)) {
					$where = "(" . $where1 . ") and (" . $where3 . ")";
				} elseif (isset($where2)) {
					$where = "(" . $where2 . ") and (" . $where3 . ")";
				} elseif (isset($where_evarsa_id)) {
					$where = "(" . $where_evarsa_id . ") and (" . $where3 . ")";
				} else {
					$where = $where3;
				}
				// veritabanı yapılandırma dosyasını dahil et
 ...
 WHERE projeler.onay='1' AND $where ORDER BY $orderby LIMIT :ilk_kayit_no, :sayfa_kayit_sayisi";
				$stmt = $con->prepare($sorgu); // sorguyu hazırla
				...
				if ($sayi > 0) {
					...
					$sorguKategori = "SELECT * FROM projeler_kategoriler";
				...
				<span aria-hidden='true'>&times;</span>
					...
					foreach ($veriEvArsa as $kayitEvArsaID) {
						if (strstr($url, "evarsa_id=" . $kayitEvArsaID['id'])) {
							$urlDuzen = str_replace("evarsa_id=" . $kayitEvArsaID['id'], "evarsa_id=4", $url);

							echo "<a style='text-decoration:none;color:#212529;' class='btn btn-danger text-white pull-left m-3'>
			<form action='" . $urlDuzen . "' method='POST'>
				Projeler <b>" . $kayitEvArsaID['ilanTuru'] . "</b>&nbsp;
				<button style='margin-top:-2px;' type='submit' class='close text-white' aria-label='Close'>
				<span aria-hidden='true'>&times;</span>
				</button>
			</form>
 ...
 <div class="card-body">
 <h4 class="card-title"><?php // echo mb_substr($kayit['urunadi'],'0','17','UTF-8')."..."
						?></h4>
 <p class="card-text"><?php // echo $kayit['sehir']."/".$kayit['ilce']
						?></p>
 <p class="card-text"><?php // echo "<b>İlan Türü:</b> ".$kayit['kategoriadi']." ".$kayit['ilanTuru']
						?></p>
 </div>
 <div class="card-footer text-muted">
 <a href="#" class="text-secondary float-left favori-ekle" id="<?php // echo $kayit['id']
																?>"><i class="fa fa-heart fa-2x"></i>Favorile</a>
 <span class="badge badge-success p-2 float-right"><?php // echo number_format($kayit['fiyat'], 0, ',', '.')
													?>&#8378;</span>
 </div>
 </div>
 </div>-->
						...
						<div class="card mb-3" style="max-width: 940px;">
							<div class="row no-gutters">
								...
								<div class="col-md-8">
									<div class="card-body">
										<a href="projedetay.php?id=<?php echo $kayit['id'] ?>" style="color:green;">
											<h5 class="card-title"><?php echo /*mb_substr(*/ $kayit['urunadi']/*,'0','50','UTF-8')."..."*/ ?></h5>
										</a>
							...
							<div class="card-footer text-muted" style="max-width: 940px;">
								<a style="text-decoration:none; color:#212529;" class='btn btn-info text-white pull-left mr-3' href="projedetay.php?id=<?php echo $kayit['id'] ?>">
									İlanı Gör
								</a>
			...
			<?php
					...
					$sorgu = "SELECT COUNT(*) as kayit_sayisi FROM projeler WHERE onay='1' AND $where";
